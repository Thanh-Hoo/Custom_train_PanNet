...
ctw_root_dir = './data/'
ctw_train_data_dir = ctw_root_dir + 'train/train_images/'
ctw_train_gt_dir = ctw_root_dir + 'train/label_train/'
ctw_test_data_dir = ctw_root_dir + 'val/test_images/'
ctw_test_gt_dir = ctw_root_dir + 'val/test_labels/'
...
class PAN_CTW(data.Dataset):
    def __init__(self,
        ...
        for data_dir, gt_dir in zip(data_dirs, gt_dirs):
            #img_names = [img_name for img_name in mmcv.utils.scandir(data_dir, '.jpg')] # to be handled by removing mmcv
            #img_names.extend([img_name for img_name in mmcv.utils.scandir(data_dir, '.png')]) # to be handled by removing mmcv
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(train_dataset):
        ...
        gt_texts = gt_texts.detach().cpu().numpy()
        gt_kernels = gt_kernels.detach().cpu().numpy()[0]
        training_masks = training_masks.detach().cpu().numpy()
        gt_instances = gt_instances.detach().cpu().numpy()
        '''
        plt.figure(1)
        plt.imshow(imgs)
        plt.figure(2)
